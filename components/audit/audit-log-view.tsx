...
import type { AuditCategory, AuditEntityType } from "@/lib/audit-db";
import { Card, CardContent, CardHeader, CardTitle } from "@/components/ui/card";
import { Button } from "@/components/ui/button";
import { Input } from "@/components/ui/input";
import { Select, SelectContent, SelectItem, SelectTrigger } from "@/components/ui/select";
import { ClipboardList, User, Calendar, Globe } from "lucide-react";
...
const ENTITY_COLORS: Record<string, string> = {
  ...
  tenant: "bg-cyan-100 text-cyan-800 dark:bg-cyan-900/30 dark:text-cyan-300",
...
function describeBrowser(userAgent: string) {
  ...
  const browser = rules.find(([pattern]) => pattern.test(userAgent));
  const version = browser ? userAgent.match(browser[0])?.[1] : null;
  const os =
  ...
  const browserLabel = browser ? `${browser[1]}${version ? ` ${version.split(".")[0]}` : ""}` : "Unknown browser";
  return os ? `${browserLabel} on ${os}` : browserLabel;
}
...
export function AuditLogView({
  ...
  const getFieldLabel = useCallback((field: string) => {
    const fieldMap: Record<string, string> = {
      ...
      locations: t("audit.field.locations"),
      extras: t("audit.field.extras"),
    };

    return fieldMap[field] ?? field;
  }, [t]);

  const formatFieldValue = useCallback((field: string, value: string | number | null) => {
    if (value === null || value === "") return t("audit.emptyValue");

    if (field === "status") {
      const fleetStatus = t(`fleet.status.${value}` as TranslationKey);
      const reservationStatus = t(`res.status.${value}` as TranslationKey);
      if (fleetStatus !== `fleet.status.${value}`) return fleetStatus;
      if (reservationStatus !== `res.status.${value}`) return reservationStatus;
    }
    ...
    if (field === "role") {
      const translated = t(`settings.users.role.${value}` as TranslationKey);
      return translated === `settings.users.role.${value}` ? String(value) : translated;
    }
    ...
    if (field === "dailyRate" || field.endsWith("Price") || field === "invoiceTotal" || field === "paidAmount") {
      return `€${value}`;
    }

    if (field === "mileage" || field === "returnMileage") {
      return `${value} km`;
    }
  ...
  const formatMetadataValue = useCallback((key: string, value: string) => {
    ...
    if (key === "period" || key === "newReturnDate") {
      return key === "period"
        ? value.split(" -> ").map((item) => formatDateTimeValue(item)).join(" -> ")
        : formatDateTimeValue(value);
    }

    if (key === "fuelLevel") {
      const fuelKey = `res.fuel${value.split("_").map((part) => part[0].toUpperCase() + part.slice(1)).join("")}` as TranslationKey;
      return t(fuelKey);
    }
    ...
    if (key === "role" || key === "plan" || key.endsWith("Price") || key === "invoiceTotal" || key === "paidAmount" || key === "returnMileage" || key === "status") {
  ...
  useEffect(() => {
    void fetchLogs(entityTypeFilter, userFilter, dateFrom, dateTo, page);
  }, [dateFrom, dateTo, entityTypeFilter, fetchLogs, page, userFilter]);
    ...
    <div className="space-y-4">
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:flex-wrap sm:items-center">
        ...
        <Input type="date" value={dateFrom} onChange={(event) => {
          ...
          setPage(1);
        }} className="w-full sm:w-44" aria-label={t("audit.fromDate")} />

        <Input type="date" value={dateTo} onChange={(event) => {
          setDateTo(event.target.value);
          setPage(1);
        }} className="w-full sm:w-44" aria-label={t("audit.toDate")} />
                  ...
                  <div key={entry.id} className="flex flex-col gap-2 px-4 py-3 sm:flex-row sm:items-start sm:gap-4">
                    ...
                    <div className="flex flex-1 flex-col gap-2">
                      <div className="flex flex-wrap items-center gap-2">
                        <span className={`inline-flex items-center rounded px-1.5 py-0.5 text-xs font-medium ${ENTITY_COLORS[entry.entityType] ?? ENTITY_COLORS.settings}`}>
                          {getEntityLabel(entry.entityType)}
                        </span>
                        <span className="text-sm font-medium">{getActionLabel(entry.action)}</span>
                      </div>

                      {(entry.ipAddress || browser) ? (
                        <div className="flex flex-wrap items-center gap-3 text-xs text-muted-foreground">
                          {entry.ipAddress ? <span>{t("audit.ipAddress")}: {entry.ipAddress}</span> : null}
                          {browser ? (
                            <span className="flex items-center gap-1">
                              <Globe className="h-3.5 w-3.5" />
                              {t("audit.browser")}: {browser}
                            </span>
                          ) : null}
                        </div>
                      ) : null}
                      ...
                      {structuredDetail?.summary ? (
                        <div className="space-y-1">
                          ...
                          {structuredDetail.note ? <p className="text-sm text-muted-foreground">{structuredDetail.note}</p> : null}
                            ...
                            <div className="space-y-1 text-sm">
                              {structuredDetail.changes.map((change, index) => (
                                <p key={`${entry.id}-${change.field}-${index}`}>
                                  ...
                                  <span className="text-muted-foreground">{formatFieldValue(change.field, change.oldValue)}</span>{" "}
                                  <span aria-hidden="true">→</span>{" "}
                                  <span>{formatFieldValue(change.field, change.newValue)}</span>
                                </p>
                              ))}
                            </div>
                          ) : null}
