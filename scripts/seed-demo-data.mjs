...
function makeVehicle(index, model, status) {
  ...
  return {
    ...
    mileage: mileageBase + Math.floor(random() * 25000),
    dailyRate,
    status,
...
function makeReservation(idNumber, vehicle, customer, startDate, durationDays, status) {
  ...
  return {
    ...
    notes: chance(0.08) ? "Demo reservation note." : "",
...
function main() {
  ...
  for (const vehicle of vehicles.slice(0, 4)) {
    reservations.push(
      makeReservation(
        reservationId++,
        vehicle,
        pick(customers),
        addDays(TODAY, -2 - Math.floor(random() * 3)),
        5 + Math.floor(random() * 7),
        "active"
      )
    );
  }
  ...
  console.log(`Seeded ${retiredVehicles.length} retired legacy vehicles in reservation history only.`);
  console.log(`Seeded ${customers.length} demo customers.`);
  console.log(`Seeded ${reservations.length} demo reservations from ${toIsoDate(START)} to ${toIsoDate(TODAY)}.`);
